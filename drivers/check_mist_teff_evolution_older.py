...
eep_paths = sorted(glob(os.path.join( '../secret_data/', 'mist', "*.track.eep")))
...
outdir = os.path.join(RESULTSDIR, "mist_param_evolution_older")
if not os.path.exists(outdir): os.mkdir(outdir)

for differential in [0]:
    for youngerages in [1]:
